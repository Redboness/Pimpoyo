...
import { useNavigate } from 'react-router-dom'; // CORREGIDO: Eliminado el guion bajo

// --- Import del componente de estadísticas ---
...
function SidePanel({ isOpen, onClose, userInfo, authToken, onLogout, onSettingsSaved }: SidePanelProps) {
  ...
  const fetchUserGlossaryTerms = useCallback(async () => {
    if (!authToken) {
      console.warn("fetchUserGlossaryTerms: No auth token found.");
      return;
    }
    ...
    try {
      ...
      const formattedData: GlossaryEntry[] = fetchedData.map(termFromApi => ({
        id: termFromApi.id,
        term: termFromApi.termino,
        definition: termFromApi.definicion,
        isDefault: false,
        userId: termFromApi.usuario_sesion_id,
        fecha_creacion: termFromApi.fecha_creacion 
  ...
  const fetchUserStats = useCallback(async () => {
    ...
    try {
        // const response = await fetch('/users/me/stats', { // Ajusta la ruta si es necesario
        //   headers: { 'Authorization': `Bearer ${authToken}` }
        // });
        // if (!response.ok) throw new Error('Failed to fetch stats');
        // const statsData = await response.json();
        // setUserStats({
        //   totalAnalizadas: statsData.total_analizadas || 0,
        //   aciertos: statsData.aciertos || 0, 
        //   fallos: (statsData.total_analizadas || 0) - (statsData.aciertos || 0), 
        //   xp: statsData.xp_actual || 0, 
        //   xpNextLevel: statsData.xp_siguiente_nivel || 100, 
        // });
  ...
  useEffect(() => {
    if (activeSection === 'stats' && authToken) {
      fetchUserStats();
    }
  }, [activeSection, authToken, fetchUserStats]);
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const bodyPayload: Partial<UserInfo> = { apodo: newNickname };
      if (newAvatarUrl) {
        bodyPayload.avatar_url = newAvatarUrl;
      } else {
        bodyPayload.avatar_url = null; 
      }
  ...
  const handleAddGlossaryTerm = async (event: React.FormEvent) => {
    ...
    if (!newTerm.trim() || !newDefinition.trim()) {
      setGlossaryError("Debes escribir un término y una definición.");
      return;
    }
    if (!authToken) {
      setGlossaryError("Error de autenticación.");
      return;
    }
    setGlossaryError(null);
    setGlossaryLoading(true);
  ...
  const handleActivityNavigation = (path: string) => {
    navigate(path);
    onClose(); 
  };
      ...
      <div className="panel-content">
        {userInfo && (
          <div className="profile-summary">
            <img 
              src={userInfo.avatar_url || `https://placehold.co/80x80/7E57C2/FFFFFF?text=${userInfo.apodo.charAt(0).toUpperCase()}`} 
              alt="Avatar" 
              className="profile-avatar-panel"
            />
            <div className="profile-info-panel">
              <span className="profile-nickname-panel">{userInfo.apodo}</span>
              {userInfo.edad && <span className="profile-age-panel">Edad: {userInfo.edad}</span>}
            </div>
          </div>
        )}
        <hr className="separator-panel"/>

        <div className="panel-nav-activities">
          <h4>Actividades</h4>
          <button onClick={() => handleActivityNavigation('/chat')} className="panel-button activity">
            Chat Pimpoyo (Detectar)
          </button>
          {/* (NUEVO BOTÓN) Para el Análisis Guiado */}
          <button onClick={() => handleActivityNavigation('/analisis-guiado')} className="panel-button activity">
            Análisis Guiado (Chatbot)
          </button>
          <button onClick={() => handleActivityNavigation('/desafio-noticias')} className="panel-button activity">
            Desafío de Noticias
          </button>
           <button onClick={() => handleActivityNavigation('/chat-libre')} className="panel-button activity">
            Chat Libre con Pimpoyo
          </button>
        </div>
        <hr className="separator-panel"/>

        <div className="panel-nav-buttons">
          <button id="btn-glossary" className={`panel-button section-toggle ${activeSection === 'glossary' ? 'active' : ''}`} onClick={() => handleSectionChange('glossary')}>GLOSARIO</button>
          <button id="btn-stats" className={`panel-button section-toggle ${activeSection === 'stats' ? 'active' : ''}`} onClick={() => handleSectionChange('stats')}>ESTADÍSTICAS</button>
          <button id="btn-settings" className={`panel-button section-toggle ${activeSection === 'settings' ? 'active' : ''}`} onClick={() => handleSectionChange('settings')}>AJUSTES</button>
        </div>
          ...
          <div id="glossary-content" className="panel-section-content" style={{ display: 'block' }}>
            ...
            <form onSubmit={handleAddGlossaryTerm} className="glossary-add-form" style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#fdf9e0', borderRadius: '8px' }}>
              ...
              <button type="submit" className="form-button primary" disabled={glossaryLoading}>
                {glossaryLoading ? (userGlossaryTerms.length === 0 && !newTerm ? 'Cargando...' : 'Guardando...') : 'Añadir Palabra'}
              </button>
            </form>
        ...
        {activeSection === 'settings' && (
          <div id="settings-content" className="panel-section-content" style={{ display: 'block' }}>
            ...
            {userInfo ? (<>
              ...
              <button id="settings-save-btn" className="panel-button primary" style={{ marginTop: '20px' }} onClick={handleSaveSettings} disabled={settingsLoading}>
            ...
            <hr className="separator-panel" style={{marginTop: '30px'}}/>
            <button id="settings-logout-btn" className="panel-button logout" onClick={onLogout}>
              Cerrar Sesión
            </button>
          </div>
        )}
