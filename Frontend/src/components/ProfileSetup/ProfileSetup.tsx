...
const preSurveyQuestions: AnyPreSurveyQuestion[] = [
    ...
    { id: 'P3_Habilidad_Tech', text: '3. En una escala del 1 (me cuesta mucho) al 5 (se me da genial), ¿cómo de bien se te da usar tecnologías como el móvil, la tablet o el ordenador? (marca SOLO UNA opción)', type: 'radio', options: ['1 - No se me da bien.', '2 - Se me da un poco bien.', '3 - Normal, me defiendo.', '4 - Bastante bien.', '5 - Se me da genial.'] },
    ...
    { id: 'P5_Habilidad_VF', text: '5. En una escala del 1 (no soy nada bueno) al 5 (¡se me da genial!), ¿cómo dirías que se te da descubrir si una noticia que ves en internet o redes sociales es verdadera o es falsa? (marca SOLO UNA opción)', type: 'radio', options: ['1 - Nada bueno/a, me cuesta muchísimo.', ' 2 - No muy bueno/a, suelo dudar.', '3 - Normal, a veces acierto y otras no.', '4 - Bastante bueno/a, suelo acertar', '5 - ¡Soy un crack!, se me da muy bien.'] },
...
/**
 * Gestiona el flujo completo de configuración de perfil, permitiendo tanto el
 * registro de nuevos usuarios como el inicio de sesión de existentes. El registro
 * es un proceso de varios pasos que incluye la recopilación de datos demográficos
 * y la realización de un pre-test inicial.
 * @param {ProfileSetupProps} props - Propiedades del componente.
 * @param {(token: string) => void} props.onAuthSuccess - Callback que se ejecuta con el token de autenticación tras un éxito.
 * @returns {React.ReactElement} La interfaz de registro/inicio de sesión.
 */
function ProfileSetup({ onAuthSuccess }: ProfileSetupProps) {
     ...
     * Itera sobre las respuestas, las compara con la clave de puntuación y suma los puntos.
     * @param {Record<string, string | string[]>} answers - Objeto con las respuestas del usuario.
     * @returns {{s1: number, s2: number, s3: number, total: number}} Objeto con las puntuaciones.
     */
    ...
    /**
     * Manejador genérico para actualizar el estado del formulario a medida que el
     * usuario interactúa con los inputs.
     * @param {React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>} event - El evento del input.
     */
    ...
    /**
     * Manejador específico para las respuestas del pre-test de tipo radio y checkbox.
     * @param {string} questionId - El ID de la pregunta.
     * @param {string} answerValue - El valor de la opción seleccionada.
     * @param {'radio' | 'checkbox'} questionType - El tipo de pregunta.
     */
    ...
    /**
     * Controla la navegación a través de los pasos del formulario de registro,
     * validando los datos del paso actual antes de avanzar.
     * @param {React.MouseEvent<HTMLButtonElement> | React.FormEvent<HTMLFormElement>} [event] - El evento que dispara la función.
     */
    ...
    /**
     * Gestiona el envío final del formulario de registro. Realiza validaciones finales,
     * construye el payload y lo envía a la API.
     * @async
    ...
    /**
     * Gestiona el envío del formulario de inicio de sesión. Valida las credenciales
     * y las envía a la API para obtener un token de autenticación.
     * @async
     * @param {React.FormEvent<HTMLFormElement>} event - El evento de envío del formulario.
     */
    ...
    /**
     * Renderiza una única pregunta del pre-test, ya sea teórica (radio/checkbox)
     * o práctica (análisis de noticia).
     * @param {AnyPreSurveyQuestion} q - El objeto de la pregunta a renderizar.
     * @returns {React.ReactElement} El JSX correspondiente a la pregunta.
     */
    ...
    /**
     * Renderiza la sección completa del pre-test, agrupando las preguntas por secciones
     * y mostrando los textos introductorios y finales.
     * @returns {React.ReactElement} El JSX de todo el formulario del pre-test.
     */
    const renderPreTest = () => {
