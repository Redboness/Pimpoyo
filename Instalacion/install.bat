...
REM Muestra el menu principal y procesa la seleccion del usuario.
:MainMenu
ECHO ===============================================================
ECHO ==           MENU DE INSTALACION DEL PROYECTO                ==
ECHO ===============================================================
ECHO Script ejecutado desde: %~dp0
ECHO ===============================================================
ECHO.
ECHO Por favor, elige una opcion:
ECHO 1. Instalar dependencias (Frontend y Backend)
ECHO 2. Crear base de datos (Configurar PostgreSQL y ejecutar script SQL)
ECHO 3. Realizar instalacion completa (Todo lo anterior + crear .env)
ECHO 4. Salir
ECHO.
SET "CHOICE="
SET /P "CHOICE=Escribe el numero de tu opcion y presiona Enter: "

IF "%CHOICE%"=="1" GOTO Option1_InstallDependencies
IF "%CHOICE%"=="2" GOTO Option2_CreateDatabase
IF "%CHOICE%"=="3" GOTO Option3_DoEverything
IF "%CHOICE%"=="4" GOTO :final_eof
ECHO Opcion no valida.
PAUSE
GOTO MainMenu

REM Maneja la opcion 1: Instalar solo dependencias.
:Option1_InstallDependencies
ECHO ===============================================================
ECHO ==         OPCION 1: INSTALAR DEPENDENCIAS                   ==
ECHO ===============================================================
CALL :InstallDependencies_Logic
IF %ERRORLEVEL% NEQ 0 GOTO :error_exit_final_START_v5
GOTO :success_exit_menu

REM Maneja la opcion 2: Crear solo la base de datos.
:Option2_CreateDatabase
ECHO ===============================================================
ECHO ==         OPCION 2: CREAR BASE DE DATOS                     ==
ECHO ===============================================================
CALL :CreateDatabase_Logic
IF %ERRORLEVEL% NEQ 0 GOTO :error_exit_final_START_v5
GOTO :success_exit_menu

REM Maneja la opcion 3: Realizar la instalacion completa.
:Option3_DoEverything
ECHO ===============================================================
ECHO ==         OPCION 3: REALIZAR INSTALACION COMPLETA           ==
ECHO ===============================================================
CALL :InstallDependencies_Logic
IF %ERRORLEVEL% NEQ 0 GOTO :error_exit_final_START_v5
ECHO.
...
IF %ERRORLEVEL% NEQ 0 GOTO :error_exit_final_START_v5
ECHO.
CALL :CreateEnvFile_Logic
IF %ERRORLEVEL% NEQ 0 (
    IF DEFINED SCRIPT_PAUSED_FOR_SECRET_KEY (
        REM El script pauso para la generacion manual de SECRET_KEY, se considera "exito con advertencia"
...
REM Logica para la instalacion de dependencias (Python/Pipenv, Node/npm).
:InstallDependencies_Logic
...
ECHO %PIPENV_INSTALL_FAIL_MSG%
exit /b 1
:PipenvStillMissing_START_v5
ECHO %PIPENV_MISSING_MSG% & ECHO [!] Reinicia terminal?
exit /b 1
...
ECHO %PYTHON_MISSING_MSG%
exit /b 1
:BackendPipfileMissing_START_v5
ECHO [X] ERROR: Pipfile no encontrado en %~dp0..\Backend\
exit /b 1
:NpmCheckFailed_START_v5
ECHO %NODE_MISSING_MSG%
exit /b 1
:FrontendPackageMissing_START_v5
ECHO [X] ERROR: Frontend\package.json no encontrado en %~dp0..\Frontend\
exit /b 1

REM Logica para la configuracion de PostgreSQL y ejecucion del script SQL.
:CreateDatabase_Logic
ECHO.
ECHO ======================================================================
...
if %errorlevel% neq 0 goto PsqlCheckFailed_START_v5_DB
ECHO [V] Comando psql encontrado.
goto DbSetup_START_v5

:PsqlCheckFailed_START_v5_DB
...
if "%PG_ADMIN_PASS%"=="" ( ECHO [X] ERROR: La contraseña de admin no puede estar vacía. & exit /b 1 )
...
goto RunSqlScript_START_v5_DB_Logic

:DbCreateUserFailed_START_v5
ECHO [X] ERROR: Falló la creación del usuario PostgreSQL '%DB_USER%'. Comprueba pass/permisos. Errorlevel: %errorlevel% & SET PGPASSWORD=
exit /b 1
:DbCreateDatabaseFailed_START_v5
ECHO [X] ERROR: Falló la creación de la base de datos PostgreSQL '%DB_NAME%'. Errorlevel: %errorlevel% & SET PGPASSWORD=
exit /b 1
:DbAlterOwnerFailed_START_v5
ECHO [X] ERROR: Falló el cambio de dueño de la base de datos '%DB_NAME%'. Errorlevel: %errorlevel% & SET PGPASSWORD=
exit /b 1
:DbGrantPermsFailed_START_v5
ECHO [X] ERROR: Falló el otorgar permiso CREATE en schema public a '%DB_USER%'. Errorlevel: %errorlevel% & SET PGPASSWORD=
exit /b 1

:RunSqlScript_START_v5_DB_Logic
ECHO.
ECHO [8] Ejecutando script de configuración del esquema (%SQL_SCRIPT_NAME%)...
IF NOT EXIST "%~dp0%SQL_SCRIPT_NAME%" goto SqlScriptMissing_START_v5_DB
...
if %errorlevel% neq 0 goto SqlScriptFailed_START_v5_DB
SET PGPASSWORD=
ECHO [V] Script de esquema de base de datos ejecutado correctamente.
ECHO.
exit /b 0

:SqlScriptMissing_START_v5_DB
ECHO %SQL_SCRIPT_MISSING_MSG%
exit /b 1
:SqlScriptFailed_START_v5_DB
ECHO [X] ERROR: Falló la ejecución del script SQL '%SQL_SCRIPT_NAME%'. Revisa errores de SQL arriba. Errorlevel: %errorlevel%
SET PGPASSWORD=
exit /b 1

REM Logica para la creacion del archivo .env.
:CreateEnvFile_Logic
SET "SCRIPT_PAUSED_FOR_SECRET_KEY="
ECHO.
...
ECHO [X] ERROR: No se pudo escribir en el archivo .env en "%ENV_FILE_PATH%". Verifica permisos.
exit /b 1
...
SET SCRIPT_PAUSED_FOR_SECRET_KEY=true
pause
exit /b 0

REM Muestra un mensaje de exito y vuelve al menu principal.
:success_exit_menu
ECHO.
ECHO ===============================================================
ECHO == Accion completada exitosamente. Volviendo al menu...      ==
ECHO ===============================================================
PAUSE
GOTO MainMenu

REM Muestra un mensaje de exito final y termina el script.
...
REM Muestra un mensaje de error final y termina el script.
...
REM Punto final de salida del script.
:final_eof
endlocal
ECHO.